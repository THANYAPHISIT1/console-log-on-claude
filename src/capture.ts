import type { ConsoleEvent, ConsoleLevel, Event, NetworkEvent } from './types.ts';
...
(() => {
  ...
  const session =
    ...
    'default';
  const levelsParam =
    dataset.levels ?? new URLSearchParams(src.split('?')[1] ?? '').get('levels') ?? '';
    ...
    window.fetch = (async function (
      ...
      try {
        ...
        const ev: NetworkEvent = {
          ...
          status,
          durationMs,
          reqHeaders: init?.headers ? headersToRecord(init.headers) : undefined,
          resHeaders: res ? headersToRecord(res.headers) : undefined,
          error,
          ts: Date.now(),
        };
        emit(ev);
  ...
  function headersToRecord(h: HeadersInit): Record<string, string> {
    ...
    if (h instanceof Headers) {
      ...
      for (const k of Object.keys(h)) out[k] = String((h as Record<string, unknown>)[k]);
    }
    return out;
  }
